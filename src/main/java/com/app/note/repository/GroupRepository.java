...
//    @Query("SELECT g.groupId,g.groupName FROM Group g WHERE g.groupName LIKE %:search% UNION SELECT g.groupId,g.groupName FROM Group g JOIN Title t ON g.groupId=t.group.groupId JOIN Note n ON n.title.titleId=t.titleId WHERE n.message LIKE %:search%")
//    List<Group> findByName(@Param("search") String search);
}
